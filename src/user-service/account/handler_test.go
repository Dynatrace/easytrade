...
// newTestRouter builds a gin engine wired to a Handler backed by the given mock adapter.
func newTestRouter(mock dbadapter.DbAdapter) *gin.Engine {
...
// seedAccount creates an account in the mock with a known password and returns its id.
func seedAccount(t *testing.T, mock *dbadapter.MockAdapter, username, password, origin string) int {
	t.Helper()
	acc, err := mock.CreateAccount(context.Background(), dbadapter.CreateAccountRequest{
		PackageId:             1,
		FirstName:             "Jane",
		LastName:              "Doe",
		Username:              username,
...
// before any adapter lookup is attempted.
func TestLogin_MissingFields_ReturnsBadRequest(t *testing.T) {
	router := newTestRouter(dbadapter.NewMockAdapter())
...
func TestLogin_ValidCredentials_ReturnsOk(t *testing.T) {
	mock := dbadapter.NewMockAdapter()
	seedAccount(t, mock, "demouser", "demopass", "WEB")
...
func TestGetAccount_InvalidId_ReturnsBadRequest(t *testing.T) {
	...
	recorder := doRequest(router, http.MethodGet, "/api/accounts/not-a-number", "")
...
func TestGetAccount_ExistingId_ReturnsOk(t *testing.T) {
	mock := dbadapter.NewMockAdapter()
	id := seedAccount(t, mock, "demouser", "demopass", "WEB")
	router := newTestRouter(mock)

	recorder := doRequest(router, http.MethodGet, "/api/accounts/"+strconv.Itoa(id), "")
