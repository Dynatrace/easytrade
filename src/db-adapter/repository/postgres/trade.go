...
func (repo *TradeRepository) GetByID(ctx context.Context, id string) (*models.Trade, error) {
	return firstOptional(repo.db.WithContext(ctx).Where(q(repository.ColID)+" = ?", id), toTrade)
}
...
func (repo *TradeRepository) GetOpen(ctx context.Context) ([]*models.Trade, error) {
	return findAll(repo.db.WithContext(ctx).Where(q(repository.ColTradeClosed)+" = ?", false), toTrade)
}
