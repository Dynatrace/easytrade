...
type balanceModel struct {
	AccountId *uuid.UUID `gorm:"primaryKey;default:gen_random_uuid()"`
	Value     float64
}
...
func (repo *BalanceRepository) GetByAccountID(ctx context.Context, accountID string) (*models.Balance, error) {
	return firstOptional(repo.db.WithContext(ctx).Where(q(repository.ColAccountID)+" = ?", accountID), toBalance)
}
